...
public class Balloon extends BaseActor { // Extends the BaseActor class.
    ...
    private float amplitude; // Change amount.
    final float fadeDuration; // Duration of fade out effect when popping balloon.
    private float initialY; // Initial Y position of balloon, before factoring in random adjustments.
    private float oscillation; // Regular variation in magnitude or position around a central point.
    private float speed; // Balloon speed.
    private float startingRemovalTime; // Starting time when balloon removal initiated.
    private float time; // Time elapsed, in seconds.
    private int offsetX; // Initial X position of balloon.
    ...
    private final void initBalloon()
    {
        ...
        setTexture( new Texture( Gdx.files.internal("assets/images/gray-balloon.png")) );
